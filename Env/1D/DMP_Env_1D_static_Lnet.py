...
class deep_mobile_printing_1d1r(gym.Env):
    ...
    def reset(self):
        ...
        return np.hstack((self.environment_memory[:,
                          ...
                          np.array([[self.count_brick]]), np.array([[self.count_step]]), np.array([[initial_position]]))) #add pos
        ...
        if action == 2: #add brick
            ...
            if done:
                ...
                observation = np.hstack((self.environment_memory[:,
                                         ...
                                         np.array([[self.count_brick]]), np.array([[self.count_step]]), np.array([[position]]))) #add pos
                reward = 0.0
