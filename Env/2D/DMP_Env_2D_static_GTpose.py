...
class deep_mobile_printing_2d1r(gym.Env):
    ...
    def reset(self):
        ...
        #
        return [np.hstack(
            (self.observation_(initial_position), np.array([[self.count_brick]]), np.array([[self.count_step]]))),initial_position]
        ...
        if action == 4:
            ...
            if done:
                ...
                observation = [np.hstack(
                    (self.observation_(position), np.array([[self.count_brick]]), np.array([[self.count_step]]))),position]
                reward = 0.0
    ...
    def render(self, axe, iou_min=None, iou_average=None, iter_times=1):
        ...
        #     os.makedirs(fig_dir)
        # plt.savefig(fig_dir+str(self.img_log)+".png")
        # self.img_log += 1
