...
class deep_mobile_printing_2d1r(gym.Env):
    ...
    def reset(self):
        ...
        self.environment_memory[:, :self.HALF_WINDOW_SIZE] = 2   ### change to 2 indicate outside the env 
        self.environment_memory[:, -self.HALF_WINDOW_SIZE:] = 2
        self.environment_memory[:self.HALF_WINDOW_SIZE, :] = 2
        self.environment_memory[-self.HALF_WINDOW_SIZE:, :] = 2
        ...
        #
        return [np.hstack(
            (self.observation_(initial_position), np.array([[self.count_brick/self.total_brick]]), np.array([[self.count_step/self.total_step]]))),initial_position]
        ...
        if action == 4:
            ...
            if done:
                ...
                observation = [np.hstack(
                    (self.observation_(position), np.array([[self.count_brick/self.total_brick]]), np.array([[self.count_step/self.total_step]]))),position]
                reward = 0.0
