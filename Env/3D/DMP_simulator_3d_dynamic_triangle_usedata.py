...
class deep_mobile_printing_3d1r(gym.Env):
    ...
    def reset(self):
        ...
        return [np.hstack(
            (self.observation_(initial_position), np.array([[self.count_brick/self.total_brick]]), np.array([[self.count_step/self.total_step]]))),
    ...
    def render(self, axe, axe2, iou_min=None, iou_average=None, iter_times=100, best_env=np.array([]), best_iou=None,
               best_step=0, best_brick=0, position_memo=[0, 0],best_plan=None):
        axe.clear()
        axe2.clear()
