...
class deep_mobile_printing_2d1r(gym.Env):
    ...
    def reset(self):
        ...
        #
        return np.hstack(
            (self.observation_(initial_position), np.array([[self.conut_brick]]), np.array([[self.count_step]]), self.input_plan.reshape(1, -1))).squeeze()
        ...
        if action == 4:
            ...
            if done:
                ...
                observation = np.hstack(
                    (self.observation_(position), np.array([[self.conut_brick]]), np.array([[self.count_step]]), self.input_plan.reshape(1, -1)))
                reward = 0.0
