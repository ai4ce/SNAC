...
class deep_mobile_printing_1d1r(gym.Env):
    def __init__(self, plan_choose=0):
        ...
        self.observation_space = spaces.Box(low=np.array([-1,-1,0,0,0,0,0]), high=np.array([99,99,99,99,99,self.total_step,self.total_step]), dtype=np.int)
        self.action_dim = 3
    ...
    def reset(self):
        ...
        return np.hstack((self.environment_memory[:,
                          ...
                          np.array([[self.conut_brick]]), np.array([[self.count_step]]))).squeeze()
