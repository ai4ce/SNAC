...
class deep_mobile_printing_3d1r(gym.Env):
    def __init__(self, data_path, random_choose_paln=True):
        ...
        self.observation_space = spaces.Box(low=np.array([-1]*int((2*self.HALF_WINDOW_SIZE+1)**2) +[0,0] + [0] * 20**2), high=np.array([99]*int((2*self.HALF_WINDOW_SIZE+1)**2) + [self.total_step,self.total_step] + [1] * (20**2)), dtype=np.int)
    ...
    def reset(self):
        ...
        return np.hstack(
            (self.observation_(initial_position), np.array([[self.count_brick]]), np.array([[self.count_step]]), self.input_plan.reshape(1, -1))).squeeze()
    ...
    def step(self, action):
        ...
        # building brick: 4 left_brick, 5 right_brick, 6 up_brick, 7 down_brick
            ...
            (self.observation_(position), np.array([[self.count_brick]]), np.array([[self.count_step]]), self.input_plan.reshape(1, -1))).squeeze()
                reward = -100.0
    ...
    def render(self, axe, axe2, iou_min=None, iou_average=None, iter_times=100, best_env=np.array([]), best_iou=None,
               best_step=0, best_brick=0,position_memo = [0,0],best_plan=np.array([])):
        axe.clear()
        axe2.clear()
        ...
        if best_plan.any():
            plan = best_plan[self.HALF_WINDOW_SIZE:self.HALF_WINDOW_SIZE + self.plan_height,
               self.HALF_WINDOW_SIZE:self.HALF_WINDOW_SIZE + self.plan_width]
        else:
            plan = self.plan[self.HALF_WINDOW_SIZE:self.HALF_WINDOW_SIZE + self.plan_height,
               self.HALF_WINDOW_SIZE:self.HALF_WINDOW_SIZE + self.plan_width]
        ...
        axe.scatter(X, Y, Z, marker='s', s=32, color='r', edgecolor="k",alpha=1)
        ...
        if x1:
            axe.bar3d(x1, y1, height, width, depth, z1, color='b',alpha=0.15, shade=True)
...
if __name__ == "__main__":
    env = deep_mobile_printing_3d1r(data_path="../../../Env/3D/data_3d_dynamic_sparse_envplan_500_val.pkl", random_choose_paln=False)
    env.reset()
    print(env.plan)
