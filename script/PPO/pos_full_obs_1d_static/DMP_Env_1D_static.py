...
class MDP_deep_mobile_printing_1d1r(gym.Env):
    def __init__(self, plan_choose=0):
        ...
        self.observation_space = spaces.Box(low=np.zeros(self.plan_width+1), high=100*np.ones(self.plan_width+1), dtype=np.int)
    ...
    def reset(self):
        ...
        return np.append(self.environment_memory, initial_position)
    ...
    def step(self, action):
        ...
        if action == 2:
            ...
            if done:
                reward = 0.0
                return np.append(self.environment_memory, position), reward, done, info
            else:
