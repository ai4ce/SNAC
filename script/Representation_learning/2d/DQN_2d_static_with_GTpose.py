...
from DMP_Env_2D_static_GTpose import deep_mobile_printing_2d1r
import pickle
import math
import matplotlib
...
OUT_FILE_NAME="DQN_2d_GTpose_"+PLAN_NAME+"_lr"+str(Lr)+"_seed_"+str(seeds)
print(OUT_FILE_NAME)
log_path="/mnt/NAS/home/WenyuHan/SNAC/Representation/2D/log/static/"+OUT_FILE_NAME+"/"
...
class DQN_AGNET():
    ...
    def choose_action(self,s):      
        state = torch.FloatTensor(s[0]).to(device)
        pose = torch.FloatTensor(np.array(s[1])).unsqueeze(0).to(device)
    ...
    def learning_process(self):
        ...
        next_pose = []
        for b in minibatch:
            current_states.append(b[0][0])
            current_pose.append(b[0][1])
            acts.append(b[1])
            rewards.append(b[2])
            next_states.append(b[3][0])   
            next_pose.append(b[3][1])
...
best_reward=0
total_steps = 0
writer = SummaryWriter('/mnt/NAS/home/WenyuHan/SNAC/Representation/2D/DQN_2d_static_GTpose')
