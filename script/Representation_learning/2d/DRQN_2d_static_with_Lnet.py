...
sys.path.append('../../../')
import utils
from DMP_Env_2D_static_Lnet import deep_mobile_printing_2d1r
...
def main(cfg):
    ...
    OUT_FILE_NAME="DRQN_2d_static_withLnet_"+PLAN_NAME+"_lr"+str(Lr)+"_seed_"+str(seeds)
    print(OUT_FILE_NAME)
    ...
    class DQN_AGNET():
        ...
        def learning_process(self):
            ...
            torch_current_pose = torch.from_numpy(current_pose).float().to(device).squeeze(1)
            ...
            torch_next_pose = torch.from_numpy(next_pose).float().to(device).squeeze(1)
            Q_s, _ = self.Eval_net.forward(torch_current_states, torch_current_pose, bsize=minibatch_size,time_step=Time_step,hidden_state=hidden_batch,cell_state=cell_batch)
            Q_s_a = Q_s.gather(dim=1,index=torch_acts[:,Time_step-1].unsqueeze(dim=1)).squeeze(dim=1)
            Q_next,_ = self.Target_net.forward(torch_next_states,torch_next_pose, bsize=minibatch_size,time_step=Time_step,hidden_state=hidden_batch,cell_state=cell_batch)
    ...
    for episode in range(N_iteration):
        ...
        while True:
            ...
            predict_next_pos[0] = round(predict_pos[0])  
            predict_next_pos[1] = round(predict_pos[1]) 
            local_memory.append((current_obs, action, r, next_obs, current_pos, predict_next_pos))
            reward_train += r
