...
parser.add_argument('--Random_seed', default=2, type=int, help='random seed')
save_path = "/mnt/NAS/home/WenyuHan/SNAC_ICLR/DQN_Lnet/model_dir_DQN3d_dynamicsparse_Lnet_croppedplan/"
...
class Q_NET(nn.Module):
    def __init__(self, ):
        ...
        self.relu = nn.ReLU()
    def forward(self, state, action, cropped_plan,bsize):
        x=torch.cat((state,cropped_plan,action),dim=1)
        x = self.fc_1(x)
        x = self.relu(x)
        x = self.fc_2(x)
        x = self.relu(x)
...
test_agent.Eval_net.load_state_dict(torch.load(save_path+"DQN_3d_dynamic_withLnet_sparse_lr0.0001_seed_"+str(seeds)+"_update_traget_period_200/Eval_net_episode_"+str(pth_number)+".pth" ,map_location=device))
