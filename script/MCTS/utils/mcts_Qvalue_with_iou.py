...
# =============================================================================
    ...
    for mmm in range(ag.rollouts):
        Qvalue_list = [] # Rewards collected along the tree for the current rollout
        ...
        while select:
            if (type(node) == DecisionNode): # DecisionNode
                ...
                # print(state_p)
                # print(obs_p)
                Q_value=Q_net(torch.FloatTensor(node.parent.obs).to(device),torch.FloatTensor(np.array([node.action])).unsqueeze(0).to(device))
        ...
        while not terminal:
            ...
            state,_ , _, terminal = env.transition(state, action, ag.is_model_dynamic)
        position, environment_memory, conut_brick, count_step = state 
        estimate=estimate+10*env.iou_MCTS(environment_memory)
        # Backpropagation
        node.visits += 1
        node = node.parent
        ...
        while node:
            ...
            if len(Qvalue_list) != 0:
                estimate = Qvalue_list.pop() + ag.gamma * estimate
