...
import itertools
import mcts_Qvalue_dynamic_inputplan
# =============================================================================
...
class UCT(object):
    ...
    def __init__(self, action_space, rollouts=100, horizon=100, gamma=0.9, ucb_constant=6.36396103068, is_model_dynamic=True):
        if type(action_space) == spaces.discrete.Discrete:
            self.action_space = list(mcts_Qvalue_dynamic_inputplan.combinations(action_space))
        else:
    ...
    def ucb(self, node):
        ...
        return mcts_Qvalue_dynamic_inputplan.chance_node_value(node) + self.ucb_constant * sqrt(log(node.parent.visits)/len(node.sampled_returns))

    def act(self, env,Q_Net,state, obs, done,device):
        return mcts_Qvalue_dynamic_inputplan.mcts_procedure(self, uct_tree_policy,Q_Net,state, obs,env, done,device)
