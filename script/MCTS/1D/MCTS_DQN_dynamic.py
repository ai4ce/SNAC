...
from DMP_Env_1D_dynamic_MCTS_obs import deep_mobile_printing_1d1r_MCTS_obs
import uct_dynamic
...
OUT_FILE_NAME="DQN_1d_sincurve_lr"+str(Lr)+"_rollouts"+str(ROLLOUT)+"_ucb_constant"+str(UCB_CONSTANT)
print(OUT_FILE_NAME)
...
while True:
    ...
    while True:   
        ...
        agent.store_memory(prev_obs, action, reward, next_obs,env.plan)
        prev_obs = next_obs
        number_steps+=1
        if done:
            break
...
for episode in range(N_iteration):
    ...
    while True:
        ...
        agent.store_memory(obs, action, r, obs_next,env.plan)
        reward_train += r
    ...
    print("\nDQN_1d_dynamic_lr0.0001_rollouts10_ucb_constant6.364")
