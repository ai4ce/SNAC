...
def main(cfg):
    ...
    OUT_FILE_NAME="DQN_MCTS_2ddynamic_"+PLAN_NAME+"_lr"+str(Lr)+"_rollouts"+str(ROLLOUT)+"_ucb_constant"+str(UCB_CONSTANT)+"_seed_"+str(seeds)
    print(OUT_FILE_NAME)
    ...
    while True:
        ...
        while True:   
            ...
            agent.store_memory(prev_obs, action, reward, next_obs,env_train.input_plan)
            prev_obs = next_obs
            number_steps+=1
            if done:
                break
    ...
    for episode in range(N_iteration):
        ...
        while True:
            ...
            agent.store_memory(obs, action, r, obs_next,env_train.input_plan)
            reward_train += r
