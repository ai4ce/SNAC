...
class Base_Agent(object):
    ...
    def get_action_size(self):
        ...
        if self.action_types == "DISCRETE": return self.environment.action_dim
    ...
    def reset_game(self):
        ...
        # for dmp project
        self.reward_history_train=[]
        self.reward_history_test=[]
        self.iou_history_train=[]
        self.iou_history_test=[]
    ...
    def run_n_episodes(self, isTest, dictPath, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        """Runs game to completion n times and then summarises results and saves model (if asked to)"""
        self.isTest = isTest
        if self.isTest:
            self.dictPath = dictPath
            self.actor_local.load_state_dict(torch.load(self.dictPath, map_location='cuda:0'))
            self.reset_game()
            self.step(isEval=self.isTest)
